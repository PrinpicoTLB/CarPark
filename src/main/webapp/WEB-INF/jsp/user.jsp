...
<ul class="pagination">
				...
				target="main"
					onclick="$('div#main').load(this.href);return false;">${users.page}</a></li>
...
<script type="text/javascript">
	...
	function addUser() {
		...
		var html = "<input id=\"judgeusername\" name=\"judgeusername\" value=\"0\" hidden=\"hidden\"/>"
				...
				+ "<div class=\"input-group\">"
				+ "<input id=\"name\" name=\"name\" placeholder=\"请输入真实姓名\" type=\"text\" class=\"form-control\">"
				+ "<span style=\"color: red;\"></span>"
				+ "</div>"
				+ "</div>"
				+ "<label>电话号码：</label><div style=\"width: 30%;\">"
				+ "<div class=\"input-group\">"
				+ "<input id=\"tel\" name=\"tel\" placeholder=\"输入电话号码\" type=\"text\" class=\"form-control\">"
				+ "</div></div>"
				+ "<label>性别：</label><div style=\"width: 30%;\">"
				+ "<div class=\"input-group\">"
		...
		$.ajax({
			...
			success:function(data){
				...
				var html ="<input  name=\"id\" value=\""+data.extend.user.id+"\" hidden=\"hidden\">"
					...
					+ "</div></div>"
					+ "<label>新密码：</label><div style=\"width: 30%;\">"
					+ "<div class=\"input-group\">"
