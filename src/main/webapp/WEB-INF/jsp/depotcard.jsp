...
function rechargeDepotCard() {
	var html = "<input id=\"payid\" name=\"payid\"  value=\"1\" hidden=\"hidden\"/>"
			...
			+ "</div>"
			+ "</div>"
			+"<label>卡类型：</label><div style=\"width: 30%;\">"
			+ "<div class=\"input-group\">"
			+ "<input id=\"cardType\" type=\"text\" class=\"form-control\" readonly>"
			+ "</div>"
			+ "</div>"
			+"<label>充值金额：</label><div style=\"width: 30%;\">"
			+ "<div class=\"input-group\">"
			+ "<input id=\"money\" name=\"money\" onblur='checkMoney()' placeholder=\"请输入金额\" type=\"text\" class=\"form-control\">"
			+ "</div>"
			+ "</div>";
...
// 	if (cardType === "普通卡" && money < 300) {
// 		alert("输入的金额必须大于300元");
// 	}
// 	if (cardType === "年卡" && money < 1000) {
// 		alert("输入的金额必须大于1000元");
// 	}
// }
	...
	$.ajax({
		...
		success:function(data){
			if (data.code == 100) {
				...
				$("#judge").val(0);
				const cardType = data.extend.cardType.type;
				document.getElementById("cardType").value = cardType;
				// if(cardType === "普通卡") {
				// 	document.getElementById('money').placeholder = '请输入金额(最低300)';
				// }
				// if(cardType === "年卡") {
				// 	document.getElementById('money').placeholder = '请输入金额(最低1000)';
				// }
			} else {
